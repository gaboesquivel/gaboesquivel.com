...
export function Project({project, full = false}: {project: any, full?: boolean  }) {
  return (
    <div>
    ...
    {project.description.split("\n").map((item, descriptionId)=> {
        return (
            <p key={`${project.slug}-${descriptionId}`} className="prose prose-neutral dark:prose-invert">
                {item}
                <br/>
            </p>
        )
    ...
    <p className='text-sm'> Stack: {' '}
      {project.technologies.map((tech, techIndex) => (
        <span key={`${project.slug}-${techIndex}`}>{tech}{techIndex !== project.technologies.length - 1 ? ", ": ""}</span>
      ))}
    </p>
