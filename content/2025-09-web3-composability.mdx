...
ERC-20, ERC-721, ERC-1155, and their extensions form the language of value transfer on-chain. They make sure your token can move across wallets, DEXs, lending protocols, and DAOs. Without this baseline, nothing else would fit together.
...
These [DeFi primitives](/project/opyn) form the foundation of modern financial applications on the blockchain.
...
Originally, composability meant “[Ethereum](/tech/ethereum) composability.” Gas costs limited use cases. Layer 2s (Optimism, Arbitrum, zkSync, Starknet) changed that, enabling cheaper execution while keeping [Ethereum](/tech/ethereum) security. Today, the same primitives exist across L2s—often connected via canonical bridges and shared governance models.
...
This is the next phase: not just a composable stack on [Ethereum](/tech/ethereum), but a composable network of chains and execution layers. A token can be issued on [Ethereum](/tech/ethereum), gain liquidity on an L2, and flow into a Cosmos appchain—without losing its identity or governance hooks.
...
For [founders and technical leaders](/lead), composability changes the economics of building:
...
Think of it like building on the internet: you don’t reimplement TCP/IP, HTTP, or SSL. You build products that sit on top of them.

## Don’t Reinvent the Wheel

When starting a new [Web3](/web3) project:
- Use existing standards (ERC-20, ERC-721, DAO frameworks).
- Use battle-tested primitives (Uniswap, Maker, Safe, Snapshot).
