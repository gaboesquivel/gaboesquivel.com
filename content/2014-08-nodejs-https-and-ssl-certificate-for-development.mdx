---
title: NodeJS HTTPS and SSL Certificate for Development
publishedAt: 2014-08-15
summary: Setting up HTTPS and SSL certificates for Node.js development
category: ['JavaScript', 'NodeJS']
tech: ['JavaScript', 'NodeJS', 'HTTPS', 'SSL']
---
...
> Nodejitsu http://docs.nodejitsu.com/articles/HTTP/servers/how-to-create-a-HTTPS-server 
> NODE PRO TIP: Note fs.readFileSync - unlike fs.readFile, fs.readFileSync will block the entire process until it completes. In situations like this - loading vital configuration data - the sync functions are okay. In a busy [server](/backend), however, using a synchronous function during a request will force the [server](/backend) to deal with the requests one by one!
