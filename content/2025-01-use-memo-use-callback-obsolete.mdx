...
In modern [React](/tech/reactjs) apps, default to not using `useMemo` and `useCallback` unless you truly need them. Overusing them adds complexity, extra memory, and sometimes worse performance.
