...
export function MyReactComponent({ myParam }: MyReactComponentParams) {
  ...
  useEffect(()=>{
    console.log('[component](/frontend) mounted')
  })
...
// types at the bottom, if there are too many consider a .type.[ts](/tech/typescript) file in the dir.
export interface MyReactComponentParams {
  myParam: boolean
}

// text and static content, this helps keep [component](/frontend) code more tidy and easy to read 
...
Basic rule is throwing errors on services and libs and catch them on the [react](/tech/react) component to display a friendly message. Async utility hooks will catch them internally removing the try/catch boilerplate in presentational code.

Use this error lib to easily work with [JS](/tech/javascript) error objects. See https://kentcdodds.com/blog/get-a-catch-block-error-message-with-typescript 
...
Within the "services" directory, you'll find plain JavaScript functions interacting with external APIs and storage systems. These functions expertly encapsulate the logic necessary for managing HTTPS requests, WebSocket communication, web storage, and [API](/backend) queries.
