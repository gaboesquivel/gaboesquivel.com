...
| Type safety   | Using [TypeScript](/tech/typescript) to enforce contracts between components |

MCP is a way to keep model I/O behind a contract: modules stay replaceable, types stay honest, and the rest of the app does not have to know which model is on the other side. For TypeScript work that is the useful part, not the brand.
